...
- 分类 URL 映射优先读取 `_config.yml` 的 `category_map`；未配置的新分类会尝试通过成对中英文文章自动推断，若需要固定英文 slug，可补一条映射
...
- 语言切换并非硬编码分类名：分类页会先尝试 `category_map` 和成对文章推断出的目标分类，再用同路径作为兜底
...
- 当前 `en` 位于第一位，因此 `/` 默认显示英文首页；如果以后想恢复中文首页，把 `zh-CN` 放回第一位即可
