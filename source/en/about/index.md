---
title: About
date: "2026-07-29 18:00:00"
lang: en
description: About Andy Cao, his research interests, and selected software projects.
comments: false
toc: false
---

<div class="profile-hero">
  <div class="profile-avatar-frame">
    <img class="profile-avatar" src="/images/CaoYueyang.png" alt="Portrait of Andy Cao">
  </div>
</div>

# Andy Cao · AI Product Engineer & Systems Builder

I turn ambiguous, unstructured problems into AI products and software systems that can be run, tested, and improved iteratively.

## About Me

I work at the intersection of product judgment, engineering execution, and applied research. Rather than stopping at concepts or demos, I focus on defining the problem clearly, turning the proposed solution into a usable system, and improving it through tests, evidence, and feedback.

I use AI extensively to accelerate research, design, and development, but I do not delegate judgment entirely to models. Explicit product specifications, traceable evidence, automated verification, and necessary human decisions are all part of reliable AI delivery.

I focus on Applied AI, AI Software, Research Software, and AI-native Product Engineering.

## Current Focus

I am studying **AI-native product delivery**: how research intent, interaction design, product specifications, context engineering, agent implementation, and automated verification can form a clear, traceable, and iterative software-delivery workflow.

## Background

My work has crossed product analysis, cross-platform applications, data and AI workflows, applied research, and systems implementation, with a consistent focus on moving uncertain problems toward working, testable outcomes.

### Education

**The University of Hong Kong**

- Master's: Department of Data and Systems Engineering

**Sun Yat-sen University**

- Bachelor's: Department of Chemistry

### Research Experience

My research interests include robotic training, human–robot interaction, and multimodal data analysis. I contributed to a study on exercise-fatigue recognition, working across experiment design, data processing, result analysis, and research documentation.

[Read the research note →](/en/2026/05/03/msc-thesis-fatigue-recognition/)

## Outside Work

I enjoy [photography](/en/gallery/), particularly landscapes, cities, and event documentation. I also make time for sports and outdoor activities.

## Contact

For more details, start with the portfolio, or reach out through GitHub.

<div class="profile-actions" aria-label="Work and public links">
  <a href="/en/work/">View Work →</a>
  <a href="https://github.com/GreatAndyC" target="_blank" rel="noopener noreferrer">GitHub ↗</a>
</div>
